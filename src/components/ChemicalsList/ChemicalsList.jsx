...
import { useState } from "react";
import ModalWrapper from "../ModalWrapper/ModalWrapper";
import { DeleteButton, List, Section, TableHeader, EmptyState } from "./ChemicalsList.styled";
...
export default function ChemicalsList({ chemicals, remove }) {
  ...
  return (
    <Section>
        ...
        {chemicals
          ...
          .map((chemical) => (
            <li key={chemical._id}>
              ...
              <span>{chemical.initialBalances}</span>
              <span>{chemical.description}</span>
              <button type="button" onClick={() => setChemicalToDelete(chemical)}>
                <DeleteButton />
              </button>
            </li>
          ))}
      ...
      >
        <p style={{ color: "black" }}>Ви дійсно хочете видалити цю запис?</p>
      </ModalWrapper>
    </Section>
  );
}
